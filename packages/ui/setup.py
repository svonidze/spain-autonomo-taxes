...
class BuildWithFrontend(build_py):
    def run(self):
        root = Path(__file__).parent / "src/autonomo_taxes_ui/dist"
        try:
            ...
            raise RuntimeError("Run npm ci --include=dev and npm run build before building the Python package") from exc
        ...
        output = Path(self.build_lib) / "autonomo_taxes_ui/dist"
