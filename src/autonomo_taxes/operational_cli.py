...
def register_operational_commands(subparsers: argparse._SubParsersAction[Any]) -> None:
    ...
    register_commands({'intake':intake_sub,'transactions':transaction_sub,'documents':document_sub,'expense':expense_sub,'review':review_sub,'counterparties':counterparty_sub})
...
def _cmd_review_apply_fx(args: argparse.Namespace) -> int:
    result, code = apply_fx(args.db, _load_json_object(args.input))
    _emit(result)
    return code
...
def _cmd_review_post_batch(args: argparse.Namespace) -> int:
    result, code = post_batch(json.load(sys.stdin), database=args.db, period=args.period, inbox_root=args.inbox_root, archive_root=args.archive_root, show_paths=args.show_paths)
    _emit(result)
    return code
...
def _cmd_period_dashboard(args: argparse.Namespace) -> int:
    result, code = dashboard(database=args.db, period=args.period, as_of=args.as_of, out_dir=args.out_dir, xolo_modelo130_calculations=args.xolo_modelo130_calculations)
    _emit(result)
    return code
...
from .services.intake_operations import _backfill_existing_intake_document, _document_suggestion, _ingest_document, _intake_transaction_draft, _intake_transaction_summary, _quarter_key, _resolve_open_intake_issue, _upsert_intake_counterparty

from .services.posting_operations import post_batch, _finalize_posted_transaction, _inbox_cleanup_error_message, _inbox_cleanup_message, _inbox_cleanup_payload, _is_sqlite_busy, _load_post_batch_request, _parse_post_batch_item, _post_batch_failure_result, _post_batch_reason_code, _post_batch_result_row, _post_batch_status, _post_transaction_review, _preflight_post_batch_period, _summarize_already_posted_transaction, _transaction_for_cleanup_output

from .services.calculation_operations import _approved_forecast_rows, dashboard, _filed_baseline, _filed_value_candidates, _nonnegative_decimal, _previous_filed_negative_carry, _previous_filed_positive, _previous_filed_vat_compensation, _tax_rows_from_db

from .services.review_operations import apply_fx

from .services.operation_utils import _parse_review_id
