...
def write_source_book_dropzone_markdown(path: Path, rows: list[dict[str, str]], *, response_root: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "## After Files Arrive",
            "",
            "Run the source-book intake commands in this order:",
            "",
            "```powershell",
            ...
            "```",
            "",
            "A quarter is not closed merely because a file exists. It closes only after imported source-book rows, asset amortization rows, and Modelo 130 tie-outs reconcile to the filed values.",
            "",
        ]
    )
