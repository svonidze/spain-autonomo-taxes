...
def write_register_answer_intake_markdown(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "",
        "Fill this from Xolo's submitted Modelo 130 register and asset schedule answers.",
        ...
        "## How To Fill",
        "",
        "- `confirmed_included`: use `yes` only if Xolo confirms the row is in the submitted register; use `no` only if Xolo confirms exclusion, netting, reversal, or non-deductible treatment.",
        ...
        "- `confirmed_source`: register export, asset schedule, Xolo support answer, or local evidence path.",
        "",
        "## First Open Rows",
        "",
...
def _application_target(classification: str) -> tuple[str, str]:
    ...
    if classification == "unresolved_after_nearest_subset":
        return (
            ...
            "Record the residual rounding, basis, or extra submitted-register adjustment explained by Xolo.",
        )
