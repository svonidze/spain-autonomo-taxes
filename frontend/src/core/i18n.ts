...
export interface LocaleSpec {code: string; name: string; intl: string; direction: 'ltr' | 'rtl'; quarterNumbers?: string[]}
...
export function normalizeLocale(value: unknown): Locale { return isLocale(value) ? value : defaultLocale; }
...
export function quarterNumber(quarter: number, locale: unknown): string {
  return localeRegistry.find(item => item.code === normalizeLocale(locale))?.quarterNumbers?.[quarter - 1] ?? String(quarter);
}
export function getLocale(): Locale { return selected; }
...
export function loadLocale(storage: () => Pick<Storage, 'getItem'>): Locale {
  try { return normalizeLocale(storage().getItem(localeStorageKey)); }
  catch { return defaultLocale; } // Storage can be disabled independently of the UI.
}
export function storeLocale(storage: () => Pick<Storage, 'setItem'>, value: unknown): void {
  try { storage().setItem(localeStorageKey, normalizeLocale(value)); }
  catch { /* The selected in-memory locale remains usable. */ }
}
...
function formatter(locale: string): IntlShape {
  ...
  if (!intl) {
    ...
    intl = createIntl({locale: tag, defaultLocale: tag, messages: catalogs.ast[locale], onError: error => {throw error;}}, cache);
...
export function formatMessage(id: string, values: Record<string, unknown> = {}, locale: unknown = selected): string {
  ...
  const normalized = Object.fromEntries(Object.entries(values).map(([key, value]) => [key,
...
export type UiMessage = {[K in MessageId]: {key: K} & (keyof MessageValues[K] extends never
  ? {params?: Record<string, never>} : {params: MessageValues[K]})}[MessageId];
type MessageParameters<K extends MessageId> = keyof MessageValues[K] extends never
  ? [params?: Record<string, never>] : [params: MessageValues[K]];
...
export function messageText(value: UiMessage | string | null | undefined, locale: unknown = selected): string {
  return value && typeof value === 'object' ? formatMessage(value.key, value.params, locale) : value ?? '';
}

/** Preserve old dictionary APIs while their consumers migrate to typed t(). */
export function legacyTemplate(id: string, locale: string): string {
  const placeholders = Object.fromEntries(Object.entries(catalogs.arguments[id] || {}).map(([name, kind]) => [name, kind === 'string' ? `{${name}}` : 0]));
  return formatMessage(id, placeholders, locale);
}

export function installLocaleControls(document: Document): void {
  const container = document.querySelector('.locale-control');
  if (!container) return;
  container.replaceChildren(...localeRegistry.map(locale => {
    const button = document.createElement('button');
    button.type = 'button';
    button.dataset.locale = locale.code;
    button.textContent = locale.code.toUpperCase();
    button.title = locale.name;
    button.setAttribute('aria-pressed', String(locale.code === selected));
    return button;
  }));
}
