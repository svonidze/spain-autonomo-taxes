...
export function reviewDraftKey(transactionId: string): string {
  return `${reviewDraftPrefix}:${transactionId || 'unknown'}`;
}
...
function safeKeys(value: unknown): boolean {
  ...
  return (
    ...
    Object.entries(value).every(
      ([key, item]) => !['__proto__', 'prototype', 'constructor'].includes(key) && safeKeys(item),
    )
  );
}
export function readReviewDraft(
  ...
  try {
    ...
    return null;
  } // Corrupt or unavailable browser storage is optional.
}
export function writeReviewDraft(storage: () => DraftStorage, draft: ReviewDraft): void {
  if (!draft.transaction_id) return;
  try {
    storage().setItem(reviewDraftKey(draft.transaction_id), JSON.stringify(draft));
  } catch {
    /* Editing remains available without persistent storage. */
  }
}
export function removeReviewDraft(storage: () => DraftStorage, transactionId: string): void {
  if (!transactionId) return;
  try {
    storage().removeItem(reviewDraftKey(transactionId));
  } catch {
    /* Browser storage may have been disabled after the draft was read. */
  }
}
