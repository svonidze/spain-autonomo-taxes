...
export function captureFocus(host: HTMLElement | null): FocusAnchor | null {
  ...
  const peers = [...host.querySelectorAll<HTMLElement>('*')].filter(
    (element) => element.tagName === active.tagName && element.className === active.className,
  );
  const anchor: FocusAnchor = {
    id: active.id,
    decisionPath: active.dataset.decisionPath,
  ...
  if (['INPUT', 'TEXTAREA'].includes(active.tagName)) {
    ...
    if (typeof field.selectionStart === 'number' && typeof field.selectionEnd === 'number')
      anchor.selection = [field.selectionStart, field.selectionEnd];
  }
  return anchor;
}
...
export function restoreFocus(host: HTMLElement | null, anchor: FocusAnchor | null): void {
  ...
  const target = anchor.id
    ? elements.find((element) => element.id === anchor.id)
    : anchor.decisionPath
      ? elements.find((element) => element.dataset.decisionPath === anchor.decisionPath)
      : anchor.href
        ? elements.find((element) => element.getAttribute('href') === anchor.href)
        : elements.filter(
            (element) => element.tagName === anchor.tag && element.className === anchor.className,
          )[anchor.ordinal];
  if (!target) return;
  ...
  if (
    ...
    (target.tagName === 'TEXTAREA' ||
      (target.tagName === 'INPUT' &&
        ['text', 'search', 'url', 'tel', 'password'].includes((target as HTMLInputElement).type)))
  )
    (target as HTMLInputElement | HTMLTextAreaElement).setSelectionRange(...anchor.selection);
}
