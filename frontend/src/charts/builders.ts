...
export function createChartBuilders(locale: Locale) {
  ...
  function taxChartEmptyMessage(
    points: (TaxPoint & { modelo130?: TaxPoint; modelo303?: TaxPoint })[],
    fallbackKey: string,
  ) {
    ...
    return filedWithoutValues ? t('charts.empty.filedWithoutValues') : t(fallbackKey);
  }
  ...
  function buildBusinessResultSpec(analytics: Analytics) {
    ...
    spec.series = [
      {
        key: 'income-actual',
        label: t('charts.business.incomeActual'),
        kind: 'bar',
        stack: 'income',
        tone: 'info',
        pattern: 'solid',
        values: monthly.actual.income_base_minor,
      },
      {
        key: 'income-backlog',
        label: t('charts.business.incomeBacklog'),
        kind: 'bar',
        stack: 'income',
        tone: 'info',
        pattern: 'hatched',
        values: monthly.approved_unposted.income_base_minor,
      },
      {
        key: 'income-forecast',
        label: t('charts.business.incomeForecast'),
        kind: 'bar',
        stack: 'income',
        tone: 'info',
        pattern: 'outline',
        values: monthly.approved_future.income_base_minor,
      },
      {
        key: 'expense-actual',
        label: t('charts.business.expenseActual'),
        kind: 'bar',
        stack: 'expense',
        tone: 'warning',
        pattern: 'solid',
        values: monthly.actual.deductible_expense_minor,
      },
      {
        key: 'expense-backlog',
        label: t('charts.business.expenseBacklog'),
        kind: 'bar',
        stack: 'expense',
        tone: 'warning',
        pattern: 'hatched',
        values: monthly.approved_unposted.deductible_expense_minor,
      },
      {
        key: 'expense-forecast',
        label: t('charts.business.expenseForecast'),
        kind: 'bar',
        stack: 'expense',
        tone: 'warning',
        pattern: 'outline',
        values: monthly.approved_future.deductible_expense_minor,
      },
    ];
    return spec;
  }
  ...
  function buildTaxDueSpec(analytics: Analytics) {
    ...
    spec.series = [
      {
        key: 'm130',
        label: t('charts.taxDue.m130'),
        kind: 'bar',
        stack: 'm130',
        tone: 'accent',
        pattern: 'solid',
        values: points.map((point) => point.modelo130.payable_minor),
      },
      {
        key: 'm303',
        label: t('charts.taxDue.m303'),
        kind: 'bar',
        stack: 'm303',
        tone: 'warning',
        pattern: 'solid',
        values: points.map((point) => point.modelo303.payable_minor),
      },
    ];
    return spec;
  }
  ...
  function buildIvaPositionSpec(analytics: Analytics) {
    ...
    spec.series = [
      {
        key: 'output',
        label: t('charts.iva.output'),
        kind: 'bar',
        stack: 'output',
        tone: 'info',
        pattern: 'solid',
        values: points.map((point) => point.output_vat_minor),
      },
      {
        key: 'input',
        label: t('charts.iva.input'),
        kind: 'bar',
        stack: 'input',
        tone: 'warning',
        pattern: 'solid',
        values: points.map((point) => point.deductible_input_vat_minor),
      },
      {
        key: 'result',
        label: t('charts.iva.result'),
        kind: 'line',
        tone: 'accent',
        pattern: 'solid',
        values: points.map((point) => point.result_minor),
      },
    ];
    return spec;
  }
  ...
  function buildReserveSpec(analytics: Analytics) {
    ...
    spec.ranges = [
      {
        key: 'recommended',
        label: t('charts.reserve.recommended'),
        tone: 'info',
        value: reserve.recommended_reserve_minor,
      },
      {
        key: 'required',
        label: t('charts.reserve.required'),
        tone: 'warning',
        value: reserve.required_tax_minor,
      },
    ];
    spec.measure = {
      key: 'available',
  ...
  function buildCumulativeNetSpec(analytics: Analytics) {
    ...
    spec.series = [
      {
        key: 'net-projected',
        label: t('charts.cumulative.projected'),
        kind: 'line',
        tone: 'accent',
        pattern: 'dashed',
        values: cumulative.projected_minor,
      },
      {
        key: 'net-actual',
        label: t('charts.cumulative.actual'),
        kind: 'line',
        tone: 'accent',
        pattern: 'solid',
        values: cumulative.actual_minor,
      },
    ];
    return spec;
  }
  ...
  function buildYearComparisonSpec(analytics: Analytics) {
    ...
    spec.bucketLabel = '';
    spec.buckets = [t('charts.yoy.income'), t('charts.yoy.deductible'), t('charts.yoy.net')];
    const previous = comparison.previous_year;
    const current = comparison.current_year;
    spec.series = [
      {
        key: 'previous',
        label: `${t('charts.yoy.previousYear')} (${previous.year})`,
        kind: 'bar',
        stack: 'previous',
        tone: 'warning',
        pattern: 'hatched',
        values: [
          previous.taxable_income_minor,
          previous.deductible_expense_minor,
          previous.net_minor,
        ],
      },
      {
        key: 'current',
        label: `${t('charts.yoy.currentYear')} (${current.year})`,
        kind: 'bar',
        stack: 'current',
        tone: 'accent',
        pattern: 'solid',
        values: [current.taxable_income_minor, current.deductible_expense_minor, current.net_minor],
      },
    ];
    return spec;
  }
    ...
    spec.rows = buckets.map((bucket) => {
      ...
      return {
        ...
        segments: [
          {
            key: 'deductible',
            label: t('charts.expenses.deductible'),
            tone: 'accent',
            pattern: 'solid',
            value: bucket.deductible_minor,
          },
          {
            key: 'non-deductible',
            label: t('charts.expenses.nonDeductible'),
            tone: 'warning',
            pattern: 'hatched',
            value: bucket.non_deductible_minor,
          },
        ],
      };
    });
  ...
  function buildCounterpartySpec(analytics: Analytics) {
    ...
    const rows = (concentration.top || []).map((entry) => ({
      ...
      segments: [
        {
          key: 'income',
          label: t('charts.counterparties.income'),
          tone: 'info',
          pattern: 'solid',
          value: entry.income_minor,
        },
      ],
    }));
        ...
        segments: [
          {
            ...
            tone: 'info',
            pattern: 'hatched',
            value: concentration.other_minor,
          },
        ],
      });
    }
  ...
  function buildAmortizationSpec(analytics: Analytics) {
    ...
    spec.series = [
      {
        key: 'amortization',
        label: t('charts.amortization.perQuarter'),
        kind: 'bar',
        stack: 'amortization',
        tone: 'accent',
        pattern: 'solid',
        values: points.map((point) => point.total_minor),
      },
    ];
    return spec;
  }
  ...
  return {
    businessResult: buildBusinessResultSpec,
    taxDue: buildTaxDueSpec,
    ...
    counterparty: buildCounterpartySpec,
