...
interface ViewGuards {canLeave?(): boolean; isDirty?(): boolean; isBusy?(): boolean;}
export interface ViewHost<C> {updateContext(context: C): boolean; dispose(): void; canLeave(): boolean; isDirty(): boolean; isBusy(): boolean;}
...
export function mountView<C extends object>(root: HTMLElement, component: Component, initial: C): ViewHost<C> {
  ...
  const app = createApp(defineComponent({setup: () => () => h(component, {context: context.value, ref: instance})}));
