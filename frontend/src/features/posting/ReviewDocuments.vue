...
const props = defineProps<{ documents: ReviewDocument[]; issues: { ui_context: HelpContext }[] }>();
const { locale, t } = useLocale();
const rows = computed(() =>
  props.documents.filter((row) =>
    ['received', 'extracted', 'needs_review'].includes(row.lifecycle_status),
  ),
);
const type = (value: string) => {
  const code = String(value || 'other_document');
  return messageIds.includes(`documents.types.${code}`)
    ? formatMessage(`documents.types.${code}`, {}, locale.value)
    : code.replaceAll('_', ' ');
};
</script>
<template>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.document_id">
            <td :data-label="t('transactions.date')">
              {{ formatDateText(row.issued_on, locale) }}
            </td>
            <td :data-label="t('documents.counterparty')">{{ row.counterparty_name || '—' }}</td>
            <td :data-label="t('fields.number')">{{ row.document_number || '—' }}</td>
            <td :data-label="t('documents.type')">{{ type(row.document_type) }}</td>
            <td :data-label="t('transactions.status')"><StatusCell :context="row.ui_context" /></td>
            <td class="amount" :data-label="t('transactions.amount')">
              {{ row.total_eur ? eur(row.total_eur, locale) : '—' }}
            </td>
            <td :data-label="t('tables.actions')">
              <a
                v-if="row.source_available"
                class="text-button"
  ...
  <section class="panel">
    <header class="panel-header">
      <h2>{{ t('review.openIssues') }}</h2>
      <small>{{ issues.length }}</small>
    </header>
    <ul v-if="issues.length" class="issues-list">
      <li v-for="(issue, index) in issues" :key="issue.ui_context.subject_id || index">
