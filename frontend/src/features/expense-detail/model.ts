...
export interface TransactionDetail {
  transaction: {
    transaction_id: string; entry_type: string; lifecycle_status: string;
    transaction_date?: string; booking_date?: string; description?: string;
    amount_original_minor?: number | null; amount_minor?: number | null;
    original_currency?: string; currency: string; amount_eur_minor?: number | null;
  };
...
export interface TaxTreatment {
  treatment_type: string; jurisdiction: string; notes?: string | null; tax_code?: string;
  deductible_irpf_minor?: number | null; deductible_vat_minor?: number | null;
  vat_investment_good?: boolean | null;
  include_modelo130?: boolean | null; include_modelo303?: boolean | null; include_modelo347?: boolean | null;
}
export interface ExpenseDetailContext {
  transactionId: string; returnUrl: string; services: ViewServices;
...
export function transactionDetail(value: unknown, id: string): TransactionDetail {
  ...
  if (!data?.transaction || data.transaction.transaction_id !== id || !/^\d{4}-Q[1-4]$/.test(data.period?.period_key)) {
