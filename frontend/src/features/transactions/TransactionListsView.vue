...
import {copyable, listUrl, type TransactionListContext, type TransactionRow, type ExpensePage} from './model.ts';
const props = defineProps<{context: TransactionListContext}>();
const {locale, t} = useLocale();
const query = ref(props.context.query), search = ref<HTMLInputElement>();
const page = shallowRef<ExpensePage>(), income = shallowRef<TransactionRow[]>([]);
const busy = ref(false), error = shallowRef<unknown>();
const kinds = ['purchase', 'amortization'] as const;
let active = true, generation = 0, timer: ReturnType<typeof setTimeout> | undefined;
let retryAppend = false, retryRefresh = false, pager: ReturnType<typeof createExpensePager>;
...
async function load(append = false, refresh = false) {
  ...
  retryAppend = append; retryRefresh = refresh;
  try {
    if (context.kind === 'expense') {
      const result = await (refresh ? pager.refresh(query.value.trim()) : pager.load(query.value.trim(), append));
      if (!result || !current(revision) || (refresh && document.querySelector('#status-help-dialog[open]'))) return;
      page.value = result;
    } else {
      const params = new URLSearchParams({period: context.period, entry_type: 'income'});
      if (query.value.trim()) params.set('q', query.value.trim());
      const rows = await context.services.request(`/api/transactions?${params}`) as TransactionRow[];
      if (!current(revision)) return;
      income.value = rows;
    }
  } catch (failure) {if (current(revision)) error.value = failure;}
  finally {if (current(revision)) {busy.value = false; context.settled();}}
}
watch(() => props.context, () => {
  generation++; clearTimeout(timer); pager?.invalidate();
...
function searchChanged() {
  ...
  busy.value = true;
  timer = setTimeout(() => {void load();}, 240);
}
function refreshVisible() {
  if (expense.value && !busy.value && document.visibilityState === 'visible' && document.activeElement !== search.value && !document.querySelector('dialog[open]')) void load(false, true);
}
...
<template>
  <div>
    <div class="table-toolbar" :class="{'expense-toolbar': expense}"><h2>{{t(expense ? 'titles.expenses' : 'titles.income')}} · {{context.period}}</h2><div class="toolbar-filters">
      <label v-if="expense"><span>{{t('expense.search')}}</span><input ref="search" id="expense-search" v-model="query" type="search" @input="searchChanged"></label>
      <input v-else ref="search" id="transaction-search" v-model="query" type="search" :placeholder="t('transactions.search')" @input="searchChanged">
      <button class="primary-button" id="view-add-entry" @click="context.openIntake"><span aria-hidden="true">+</span> {{t('common.add')}}</button>
    </div></div>
    <template v-if="expense">
      <div id="expense-results"><template v-if="page"><section v-for="kind in kinds" :key="kind" class="panel expense-section" :aria-labelledby="`expenses-${kind}`"><header class="expense-section-header"><h3 :id="`expenses-${kind}`">{{t(kind === 'purchase' ? 'expense.purchase' : 'expense.amortization')}}</h3>
        <p>{{t('expense.quarterTotal', {amount: eur(page.summary[kind].reviewed_total.amount_eur, locale)})}}</p><p v-if="page.summary[kind].reviewed_total.missing_amount_count">{{t('expense.missing')}}</p><p v-if="kind === 'amortization'" class="expense-explanation">{{t('expense.explanation')}}</p>
        <div class="expense-terms"><span>{{t('transactions.status')}} <TermHelp term="posting"/></span><span>IRPF <TermHelp term="IRPF"/></span><span v-if="kind === 'purchase'">IVA <TermHelp term="IVA"/></span></div>
        <small>{{t('expense.shown', {shown: page.rows.filter(row => row.expense_kind === kind).length, count: page.matching_counts[kind]})}}</small>
      </header><ExpenseTable v-if="page.rows.some(row => row.expense_kind === kind)" :rows="page.rows.filter(row => row.expense_kind === kind)" :kind="kind" :period="context.period" :return-url="sourceUrl" :navigate="context.services.navigate"/><div v-else class="empty-state">{{t(page.matching_counts[kind] ? 'expense.moreRecords' : page.period_counts[kind] ? 'expense.noMatches' : 'expense.empty')}}</div></section></template></div>
