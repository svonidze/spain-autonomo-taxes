  ...
  for (const [key, value] of Object.entries(override)) {
    ...
    result[key] =
      record(value) && !Array.isArray(value)
        ? merge(record(base) ? base[key] || {} : {}, value)
        : clone(value);
  }
  return result as T;
}
export function setDecision(target: Record<string, unknown>, path: string, value: unknown) {
  ...
  for (let index = 0; index < parts.length - 1; index++) {
    ...
    if (!record(cursor[key])) throw new Error('Invalid field parent');
    cursor = cursor[key] as Record<string, unknown>;
  }
  cursor[parts.at(-1)!] = value;
}
export function getDecision(target: unknown, path: string): unknown {
  let result = target;
...
export function evaluate(item: WorkItem | null | undefined) {
  ...
  if (!projection)
    return {
      ...
      canApply: false,
      availableOn: null,
      reasonKey: 'review.unknownSupport',
    };
  const supported = item?.supported !== false,
    code = item?.ui_context?.state || projection.preview_bucket;
  return {
    category: !supported
      ? 'blocked'
      : code === 'deferred'
        ? 'later'
        : ['ready', 'blocked', 'needs_review'].includes(code || '')
          ? code!
          : 'blocked',
    supported,
    canApply: item?.review_allowed === true,
    availableOn: projection.posting_deferred_until || null,
    reasonKey: supported ? null : 'review.unsupported',
  };
}
export function answers(
  ...
  const party = state.counterparty || {};
  return {
    business_purpose: !!text(decision.business_purpose),
    tax_code: !!decision.tax_treatment?.tax_code,
    deductible_irpf_minor: decision.tax_treatment?.deductible_irpf_minor != null,
    asset_decision: !!decision.asset_decision,
    vat_investment_good: typeof decision.tax_treatment?.vat_investment_good === 'boolean',
    counterparty_country:
      !!text(decision.counterparty_changes?.country_code) ||
      (!!party.country_code && party.country_code !== 'ZZ'),
    fx_rate: !!fx,
    reason: !!text(decision.reason),
    document_valid: typeof decision.document_valid === 'boolean',
  };
}
export function autoResolve(
  ...
  packet.decision.issue_resolutions?.forEach((resolution, index) => {
    ...
    const issue = resolution.issue_id
      ? packet.state.issues?.find((row) => row.validation_issue_id === resolution.issue_id)
      : packet.state.issues?.[index];
    const covering = issue && coverage[issue.issue_code];
    if (covering?.length && covering.every((id) => !!answered[id])) {
      resolution.action = 'resolve';
      if (!resolution.reason)
        resolution.reason = formatMessage(
          'review.issueAutoResolveHint',
          { labels: covering.length },
          locale,
        );
    }
  });
...
export function legacyErrorTarget(message: string) {
  ...
  if (text.includes('business_purpose') || text.includes('business purpose'))
...
export function errorTarget(value: unknown) {
  return value instanceof ApiError && value.field && questionIds.has(value.field)
    ? value.field
    : legacyErrorTarget(value instanceof Error ? value.message : String(value));
}
export function needsFx(transaction: ReviewState['transaction']) {
  return (
    String(transaction.original_currency || transaction.currency || 'EUR').toUpperCase() !==
      'EUR' && !transaction.fx_rate_id
  );
}
export function initialFx(suggestion?: FxSuggestion): FxChoice | null {
  return suggestion && ['exact', 'prior'].includes(suggestion.status) ? { mode: 'ecb' } : null;
}
export function confirmFx(choice: FxChoice | null, suggestion?: FxSuggestion) {
  if (!choice) return null;
  if (choice.mode === 'ecb' && suggestion)
    return {
  ...
  const rate = String(choice.rate || '').trim(),
    rateDate = String(choice.rateDate || '').trim(),
    reference = String(choice.sourceReference || '').trim();
  ...
  return {
    ...
    raw_observation: JSON.stringify({
