...
export interface WorkItem {
  ...
  ui_context: HelpContext & {
    posting?: { preview_bucket?: string; posting_deferred_until?: string };
  };
  guidance?: {
    issue_coverage?: Record<string, string[]>;
    auto_filled?: {
      business_purpose?: string;
  ...
  fx_suggestion?: FxSuggestion;
  requirements?: (string | { code: string; supported?: boolean })[];
}
export interface GuidedContext {
  ...
  resolved(period: ReviewState['period']): string;
  complete(outcome: 'approve' | 'reject'): void;
  posted(transactionId: string, period: string, replace?: boolean): void;
  settled(): void;
  notify(message: UiMessage | string, error?: boolean): void;
}
export interface ReviewDetailContext extends GuidedContext {
  period: string;
}
