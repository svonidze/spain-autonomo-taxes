...
export interface ContactOperation {
  ...
  ui_context: HelpContext & { documents?: { document_id: string; available: boolean }[] };
  expense_kind?: string;
  deductible_irpf_eur?: number | null;
...
export interface ContactsContext {
  ...
  settled(): void;
  detailResolved(party: Counterparty): void;
  notify(message: UiMessage, error?: boolean): void;
  chartPeriod: string;
  restorePosition(): void;
  rememberPosition(): void;
}
export const contactUrl = (id: string, period = '') =>
  `/contacts/${encodeURIComponent(id)}${period ? `?period=${encodeURIComponent(period)}` : ''}`;
