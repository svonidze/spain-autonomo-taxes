...
export function createFinancePresentation(locale: Locale) {
  const t = (key: string, params: Record<string, unknown> = {}) => formatMessage(key, params, locale);
...
function formCardData(form: FormView | undefined, obligation?: Obligation, {warnOnMissingHeadline = false} = {}) {
  const fallback = form || {
    form_code: "",
...
function taxHeadlineModel(periodState: {phase?: string} | undefined, summary: TaxSummary | undefined) {
...
return {formCardData, formCardValue, formAccentClass, formSubtitle, formEmptyState, taxSettlementLabel, taxStatusTone, taxHeadlineModel};
}
