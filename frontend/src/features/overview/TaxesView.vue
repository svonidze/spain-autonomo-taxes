...
const undecided = computed(
  () => data.value?.obligations.filter((row) => row.determination === 'unknown') || [],
);
const other = computed(
  () =>
    data.value?.obligations.filter((row) => !['due', 'unknown'].includes(row.determination)) || [],
);
...
<template>
  ...
  <div v-else-if="data" class="tax-page">
    ...
    <template
      v-for="group in [
        { key: 'due', rows: extraDue, title: t('taxes.additionalDueForms'), open: true },
        { key: 'undecided', rows: undecided, title: t('taxes.undecidedForms'), open: true },
        { key: 'other', rows: other, title: t('taxes.otherForms'), open: false },
      ]"
      :key="group.key"
