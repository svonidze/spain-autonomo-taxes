...
test('draft storage is scoped to a transaction and retains false, null and zero', () => {
  ...
  const draft = {
    review_id: 'transaction:synthetic',
    transaction_id: 'synthetic',
    snapshot_hash: 'snapshot',
    decision: { approved: false, amount: 0, unknown: null },
  };
...
test('drafts cannot introduce prototype keys into a decision merge', () => {
  const raw =
    '{"review_id":"r","transaction_id":"x","snapshot_hash":"s","decision":{"__proto__":{"polluted":true}}}';
  expect(
    readReviewDraft(() => ({ getItem: () => raw, setItem() {}, removeItem() {} }), 'x'),
  ).toBeNull();
});

test('focus restoration retains selection and never steals focus from another control', () => {
  document.body.innerHTML =
    '<main><textarea id="draft">synthetic text</textarea></main><button id="outside">outside</button>';
