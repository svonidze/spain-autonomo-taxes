...
function fixture(pending = false) {
  return {
    transaction: {
      ...
      amount_original_minor: 12345,
      amount_eur_minor: 11000,
    ...
    document: {
      ...
      document_number: 'SYN-EXPENSE',
    },
    workflow_follow_up: { follow_up_pending: pending },
    tax_treatments: [
      {
        treatment_type: 'expense',
        jurisdiction: 'ES',
        notes: '<script>synthetic escaped note</script>\nsecond line',
        deductible_irpf_minor: 0,
        deductible_vat_minor: null,
        include_modelo130: true,
      },
      {
        treatment_type: 'adjustment',
        jurisdiction: 'ES',
        notes: 'Second treatment note',
...
test('Vue expense uses authoritative period, preserves notes and values, and returns to list query', async ({
  page,
}) => {
  let reads = 0;
  ...
  await expect(page.locator('.expense-note-text')).toHaveText([
    '<script>synthetic escaped note</script>\nsecond line',
    'Second treatment note',
  ]);
  ...
  expect(reads).toBe(1);
  await expect(page.locator('.expense-detail a[target="_blank"]')).toHaveAttribute(
    'rel',
    'noreferrer',
  );
...
test('follow-up POST remains single and busy across locale changes, then reloads detail', async ({
  page,
}) => {
  let posted = false,
    writes = 0,
  ...
  await page.route(`**/api/transactions/${ID}`, (route) => {
    reads++;
    return route.fulfill({ json: fixture(!posted) });
  });
...
test('follow-up failure is retryable and a late success cannot change another route', async ({
  page,
}) => {
  let writes = 0,
    release!: () => void;
  ...
  await page.route(`**/api/expense-workflows/${ID}/follow-up`, async (route) => {
    ...
    if (writes === 1)
      return route.fulfill({
        status: 409,
        json: { error: 'Synthetic follow-up conflict', code: 'follow_up_conflict' },
      });
    await gate;
    await route.fulfill({ json: { status: 'ok' } });
  });
...
test('missing, forbidden and wrong-type detail states retain usable navigation', async ({
  page,
}) => {
  await page.route(`**/api/transactions/${ID}`, (route) =>
    route.fulfill({
      status: 404,
      json: { error: 'Synthetic missing', code: 'transaction_not_found' },
    }),
  );
  await page.goto(DETAIL);
  await expect(page.locator('.expense-detail [role="alert"]')).toBeVisible();
  await expect(page.locator('.expense-detail a').first()).toHaveAttribute(
    'href',
    '/expenses?period=2026-Q3&q=Synthetic',
  );
  await page.unroute(`**/api/transactions/${ID}`);
  await page.route(`**/api/transactions/${ID}`, (route) =>
    route.fulfill({ status: 403, json: { error: 'Synthetic denied', code: 'session_forbidden' } }),
  );
  await page.reload();
  ...
  await page.unroute(`**/api/transactions/${ID}`);
  const data = fixture();
  data.transaction.entry_type = 'income';
...
test('a direct link with a non-canonical UUID loads and is rewritten to the canonical id', async ({
  page,
}) => {
  const canonical = 'abcdef01-2345-4678-8abc-def012345678';
  const data = fixture();
  data.transaction.transaction_id = canonical;
