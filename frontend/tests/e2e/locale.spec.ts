...
import chartMessages from '../../src/locales/en/charts.json' with { type: 'json' };
...
async function guided(page: Page) {
  ...
  await page.route(`**/api/transactions/${ID}`, (route) => {
    reads++;
    return route.fulfill({ json: { transaction, period: { period_key: '2026-Q3' } } });
  });
    ...
    return route.fulfill({
      json: {
        packet: {
          ...
          snapshot_hash: 'synthetic-review-snapshot',
          state: {
...
async function language(page: Page, locale: string) {
  await page
    .locator(`[data-locale="${locale}"]`)
    .first()
    .evaluate((element: HTMLElement) => element.click());
  await expect(page.locator('html')).toHaveAttribute('lang', locale);
}

test('guided fields, rejection draft and focus survive a presentation-only locale change', async ({
  page,
}) => {
  ...
  await reason.evaluate((field: HTMLTextAreaElement) => {
    field.focus();
...
test('locale change during a POST does not orphan its result or submit twice', async ({ page }) => {
  ...
  await page.route('**/api/review/confirm', async (route) => {
    ...
    await route.fulfill({
      status: 400,
      json: { error: 'Synthetic rejection failed', code: 'review_packet_invalid' },
    });
  });
  await page.locator('.review-reject-panel > summary').click();
  await page.locator('#review-reject-document-valid').selectOption('false');
  ...
  expect(writes[0]).toMatchObject({
    packet: {
      decision: { outcome: 'reject', document_valid: false, reason: 'Synthetic rejection' },
    },
  });
});

test('the native expense workflow translates without reloading or losing its unsaved payload', async ({
  page,
}) => {
  let reads = 0;
  await page.route(`**/api/transactions/${ID}`, (route) =>
    route.fulfill({
      json: {
        transaction: { transaction_id: ID, entry_type: 'expense', lifecycle_status: 'received' },
        period: { period_key: '2026-Q3' },
      },
    }),
  );
  await page.route(`**/api/document/${DOCUMENT}/preview`, (route) =>
    route.fulfill({ contentType: 'text/plain', body: 'Synthetic original' }),
  );
  await page.route(`**/api/expense-workflows/${ID}`, (route) => {
    ...
    return route.fulfill({
      json: {
        editable: true,
        draft_version: 1,
        current_snapshot_hash: 'synthetic-expense-snapshot',
        source_snapshot_hash: 'synthetic-expense-snapshot',
        source: {
          assets: [],
          issues: [],
        ...
        activities: [],
        allowed_values: { tax_code: ['domestic_input'] },
        payload: {
          facts: {
  ...
  await expect(page.locator('[data-p="decision.business_purpose"]')).toHaveValue(
    'Changed private draft',
  );
  await expect(page.locator('[data-p="supplier.display_name"]')).toHaveValue(
    'Synthetic draft supplier',
  );
  expect(reads).toBe(before);
});
...
test('pseudolocale comes from the catalog registry and leaves record data intact', async ({
  page,
}) => {
  test.skip(
    process.env.AUTONOMO_PSEUDO !== '1',
    'Run npm run test:pseudo for a test-only third locale',
  );
