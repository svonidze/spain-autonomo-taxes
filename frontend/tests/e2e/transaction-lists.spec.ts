...
function row(id = ID, kind = 'expense', label = 'Synthetic entry') {
  return {
    ...
    document_status: 'recorded',
    transaction_date: '2026-07-02',
    period_key: '2026-Q3',
    description: label,
    document_number: 'SYN-123',
    counterparty_name: label,
    amount_eur: '90.00',
    amount_original: '100.00',
    currency: 'USD',
    deductible_irpf_eur: '90.00',
...
function expensePage(
  ...
  return {
    ...
    has_more: more,
    next_offset: offset,
    matching_counts: { purchase: 2, amortization: 0 },
    period_counts: { purchase: 2, amortization: 0 },
    summary: {
      purchase: { reviewed_total: { amount_eur: '180.00', missing_amount_count: 0 } },
      amortization: { reviewed_total: { amount_eur: '0.00', missing_amount_count: 0 } },
    },
  };
}
test('income search ignores old replies and keeps current records on a failed retry', async ({
  page,
}) => {
  let release!: () => void,
    oldStarted = false;
  ...
  await page.route(/\/api\/transactions\?/, async (route) => {
    ...
    if (q === 'old') {
      oldStarted = true;
      await gate;
      return route.fulfill({ json: [row(ID, 'income', 'Old reply')] });
    }
    if (q === 'fail')
      return route.fulfill({ status: 503, json: { error: 'Synthetic search unavailable' } });
    return route.fulfill({
      json: [row(ID, 'income', q === 'new' ? 'New reply' : 'Initial reply')],
    });
  });
  await page.goto('/income?period=2026-Q3');
  await expect(page.locator('#transactions-table')).toContainText('Initial reply');
  await page.locator('#transaction-search').fill('old');
  await expect.poll(() => oldStarted).toBe(true);
  await page.locator('#transaction-search').fill('new');
  await expect(page.locator('#transactions-table')).toContainText('New reply');
  release();
  await page.locator('#transaction-search').fill('fail');
  await expect(page.locator('[data-vue-owned] [role="alert"]')).toContainText(
    'Synthetic search unavailable',
  );
  await expect(page.locator('#transactions-table')).toContainText('New reply');
  await expect(page.locator('#transactions-table')).not.toContainText('Old reply');
});
test('income copy preserves original values and cancellation leaves an untouched saved draft', async ({
  page,
}) => {
  const source = row(ID, 'income', 'Synthetic copy');
  source.amount_original = '0.00';
  await page.route(/\/api\/transactions\?/, (route) => route.fulfill({ json: [source] }));
  await page.goto('/income?period=2026-Q3');
  await page.evaluate(() =>
    localStorage.setItem(
      'autonomo.intake-draft',
      JSON.stringify({ schema: 1, values: { document_number: 'SYN-OLDER' } }),
    ),
  );
  ...
  await expect(page.locator('#vue-intake-form [name="document_number"]')).toHaveValue('SYN-OLDER');
  await page.locator('#vue-close-dialog').click();
  await page.locator('[data-copy-transaction-id]').click();
...
test('expense paging restarts a changed revision and search preserves the detail return query', async ({
  page,
}) => {
  let changed = false;
  ...
  await page.route(/\/api\/expenses\?/, (route) => {
    ...
    offsets.push(offset);
    if (offset === 1) changed = true;
    const name = `${changed ? 'New' : 'Old'} ${offset}`;
    return route.fulfill({
      json: expensePage(
        changed ? '2' : '1',
        [row(offset ? OTHER : ID, 'expense', name)],
        offset === 0,
        offset + 1,
      ),
    });
  ...
  await expect(page.locator('.expense-document-link').first()).toHaveAttribute(
    'href',
    /returnTo=.*Synthetic/,
  );
...
test('expense refresh retains expanded rows, defers during help, and disposes polling', async ({
  page,
}) => {
  ...
  await page.route(/\/api\/expenses\?/, (route) => {
    ...
    return route.fulfill({
      json: expensePage('1', [row(offset ? OTHER : ID)], offset === 0, offset + 1),
    });
  });
  await page.goto('/expenses?period=2026-Q3');
  await page.locator('#expense-more').click();
