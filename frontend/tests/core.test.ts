...
import {t, formatMessage, loadLocale, storeLocale, setLocale, subscribeLocale} from '../src/core/i18n.ts';
...
describe('locale messages and private values', () => {
  test.each([[0, '0 операций'], [1, '1 операция'], [2, '2 операции'], [5, '5 операций'], [11, '11 операций'], [21, '21 операция']])('Russian count %s', (count, expected) => {
  ...
  test('disabled storage and subscriptions do not affect data', () => {
    const denied = () => {throw new Error('disabled');};
...
describe('HTTP boundary', () => {
  const context = (payload: HttpResponse) => ({fetch: async () => payload, origin: 'https://example.invalid', t: formatMessage});
  test('preserves conflict metadata rather than normalizing it away', async () => {
    const request = fetchJSON('/api/test', {}, context(response(409, 'application/json', JSON.stringify({error: 'Synthetic conflict', code: 'stale_counterparty', current: {row_version: 3}}))));
    await expect(request).rejects.toMatchObject({status: 409, code: 'stale_counterparty', current: {row_version: 3}});
    await expect(request).rejects.toBeInstanceOf(ApiError);
  });
  test.each([[200, 'text/html', '<html>wrong service</html>'], [200, 'application/json', '{broken'], [502, 'text/plain', 'unavailable']])('rejects invalid response %s %s', async (status, contentType, body) => {
    await expect(fetchJSON('/api/test', {}, context(response(status, contentType, body)))).rejects.toBeInstanceOf(ApiError);
  });
