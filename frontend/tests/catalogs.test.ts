...
test('catalog validation detects duplicates before JSON parsing loses them', () => {
  expect(() => readMessages('{"common.label":"A","common.label":"B"}', 'fixture')).toThrow(
    'duplicate',
  );
  ...
  const definitions = [
    {
      code: 'ru',
      intl: 'ru-RU',
      message: '{count, plural, one {# запись} few {# записи} many {# записей} other {# записи}}',
    },
    { code: 'en', intl: 'en-GB', message: '{count, plural, one {# record} other {# records}}' },
    {
      code: 'es',
      intl: 'es-ES',
      message: '{count, plural, one {# registro} many {# registros} other {# registros}}',
    },
  ];
  try {
    writeFileSync(
      ...
      JSON.stringify(
        definitions.map(({ code, intl }) => ({ code, intl, name: code, direction: 'ltr' })),
      ),
    );
    for (const locale of definitions) {
      mkdirSync(join(directory, locale.code));
      writeFileSync(
        join(directory, locale.code, 'common.json'),
        JSON.stringify({ 'common.count': locale.message }),
      );
    }
    ...
    writeFileSync(
      join(directory, 'es', 'common.json'),
      JSON.stringify({ 'common.count': '{otherCount} registros' }),
    );
