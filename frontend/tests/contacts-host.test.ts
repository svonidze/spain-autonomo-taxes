...
test('host context replacement respects dirty and pending rename; saved names refresh the chart', async () => {
  ...
  input.value = 'Synthetic corrected'; input.dispatchEvent(new Event('input', {bubbles: true})); await nextTick();
  expect(host.updateContext({...context, contactId: id})).toBe(false);
  expect(confirm).toHaveBeenCalledTimes(1); expect(input.value).toBe('Synthetic corrected');
  const form = document.querySelector('#vue-counterparty-name-dialog form')!;
  form.dispatchEvent(new Event('submit', {bubbles: true, cancelable: true})); await nextTick();
