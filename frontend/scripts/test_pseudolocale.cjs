...
function run(args, env = process.env) {
  const result = spawnSync(process.execPath, args, {cwd: frontendRoot, stdio: 'inherit', env});
...
try {
  run([path.resolve(__dirname, 'build_locales.mts'), '--pseudo']);
  run([vite, 'build']);
  run([path.resolve(__dirname, 'write_ui_manifest.cjs')]);
