...
const localeDirectory = resolve(frontendRoot, 'src/locales');
const generatedDirectory = resolve(frontendRoot, 'src/generated');
...
export function generate(directory = localeDirectory, destination = generatedDirectory, pseudo = false) {
  const result = compileCatalogs(directory);
  if (directory === localeDirectory) {
    const references = JSON.parse(readFileSync(resolve(frontendRoot, 'src/help/references.json'), 'utf8'));
