...
if [[ ! -d "$target" ]]; then
  ...
  python_project="$(python3 "$script_dir/prepare_ui_release.py" python-project-path "$source_repo" "$sha" "$target")"
...
if [[ "${AUTONOMO_ENABLE_STORAGE_MIGRATION:-0}" == "1" ]]; then
  ...
  [[ -f "$migration_tool" ]] || migration_tool="$script_dir/backup/backup_sqlite.py"
