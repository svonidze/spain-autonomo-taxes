...
test('intake keeps selected file and typed fields across a locale change', async ({page}) => {
  ...
  await page.locator('[data-locale="en"]').first().evaluate((element: HTMLElement) => element.click());
  await expect(page.locator('html')).toHaveAttribute('lang', 'en');
  await expect(dialog).toBeVisible();
  await expect(dialog.locator('[name="document_number"]')).toHaveValue('SYNTHETIC-UI-1');
  expect(await dialog.locator('input[type="file"]').evaluate((element: HTMLInputElement) => element.files?.[0]?.name)).toBe('synthetic.txt');
});

test('unknown routes and missing resources never masquerade as a working page', async ({request}) => {
